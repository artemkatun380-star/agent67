window.onload = function () {
  ...
  class Particle {
    constructor(x, y, color, speed, life, size = 3) {
      ...
      this.life = life; // оставшееся время жизни
      this.maxLife = life;
      this.color = color;
      this.size = size;
    }
  ...
  // --- TOUCH УПРАВЛЕНИЕ (водим пальцем – целится, отпускаем – выстрел) ---
  ...
  canvas.addEventListener(
    ...
    function (e) {
      e.preventDefault();
      // Выстрел, если был touchstart (координаты определены)
      if (mouseX !== undefined && mouseY !== undefined) {
        // Вызываем ту же логику выстрела, что и при клике
        const dx = mouseX - lastMuzzle.x;
        const dy = mouseY - lastMuzzle.y;
        const distance = Math.sqrt(dx * dx + dy * dy);
        if (distance >= 5 && bulletsLeft > 0 && !levelComplete && !gameOver) {
          const dirX = dx / distance;
          const dirY = dy / distance;
          const speed = 10;
          bullets.push({
  ...
  function gameLoop() {
    ...
    // Обновление и отрисовка частиц взрыва (исправленный обход)
    for (let i = explosionParticles.length - 1; i >= 0; i--) {
      const p = explosionParticles[i];
      p.update();
      if (p.isDead) {
        explosionParticles.splice(i, 1); // удаляем мёртвую частицу
      } else {
        p.draw(ctx);
      }
    }

    // Обновление и отрисовка искр (аналогично)
